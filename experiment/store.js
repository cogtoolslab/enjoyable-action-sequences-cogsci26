...
function markStimulusServed(collection, gameid, stimulusId) {
    collection.update({ _id: ObjectID(stimulusId) }, {
        $push: { games: gameid },
        $inc: { numGames: 1 }
    }, function (err, items) {
        if (err) {
            console.log(`error marking stimulus as served: ${err}`);
        } else {
            console.log(`\t successfully marked stimulus as served. result: ${JSON.stringify(items)}`);
        }
    });
}
...
function serve() {
    ...
    mongoConnectWithRetry(2000, (connection) => {
        ...
        app.post('/db/insert', (request, response) => {
            ...
            log(`inserting data: ${JSON.stringify(data).substring(0, 100)}`);
            collection.insert(data, (err, result) => {
                if (err) {
                    return failure(response, `error inserting data: ${err}`);
                } else {
                    return success(response, `successfully inserted data. result: ${JSON.stringify(result).substring(100, 240)}`);
                }
            });
        });
        ...
        app.post('/db/getstims', (request, response) => {
            ...
            // Sort by number of times previously served and take the first.
            collection.aggregate([
                ...
                if (err) {
                    ...
                    // Immediately mark as served so others are less likely to receive it.
                    try {
                        markStimulusServed(collection, request.body.gameid, results[0]['_id']);
                    }
                    catch (err) {
            ...
            console.log(`Sent stim for GAMEID ${request.body.gameid}`);
        });
        ...
        app.post('/db/exists', (request, response) => {
            ...
            // Legacy helper endpoint; the main experiment does not call this route.
            var collectionList = request.body.colnames || request.body.collections || ['fun-puzzles'];
